...
Nonfiction: nothing uncited — every spoken print or claim must appear in a Parallel cite (hit URL + excerpt/summary) or a stamped finding from those cites (grounded or timeline_event). Cite timeline_event findings for narrative milestones. A named-host or named-entity claim requires a stamp whose URL host or title/claim carries that name; if none matches, drop the claim or HOLD that beat — do not attach a weakly overlapping hit as cover. If no grounded or timeline_event stamp exists for a spoken claim, drop the claim. Every sourced beat must carry those finding ids (or pack cite URLs) on the beat — do not ship untagged factual VO.
...
Tone is writer stance only — do not narrate the tone line as VO filler. Speak the stamped print/claim/note, not the article title. On-screen frames must show that named print/object (mute-test), not meta chrome such as "Cited event on screen."
